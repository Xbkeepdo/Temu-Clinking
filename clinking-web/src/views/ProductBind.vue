<template>
  <div class="product-page">
    <div class="search-container">
      ...
      />
      <button @click="handleTimeSearch">更新销售件数</button>

      <!-- 销售总和展示 -->
      <div class="total-section">销售总计：{{ totalSales.toFixed(2) }} 件</div>
    </div>
    ...
    <table class="product-table">
      <thead>
        <tr>
          <th style="width: 130px">商品图片</th>
          <th style="width: 200px">商品标题</th>
          <th style="width: 100px">skcId</th>
      ...
      <tbody>
        <tr v-for="product in filteredProducts" :key="product.id">
          ...
          <td>{{ product.salesCount.toFixed(2) }}</td>
          <td>{{ product.dailySales.toFixed(2) }}</td>
...
interface Product {
  ...
  salesCount: number; // 新增的字段：商品销售件数
  dailySales: number;
  totalCost: number;
  designerList: string[];
}
...
const searchParams = ref({
  startTime: formatDate1(oneMonthAgo),
  endTime: formatDate1(now),
});

// const daysBetween = computed(() => {
//   if (!timeRange.value[0] || !timeRange.value[1]) return 30;
//   const diff =
...
const startPickerOptions = computed(() => ({
  disabledDate: (time) => {
    if (!searchParams.value.endTime) return false;
    const endDate = new Date(searchParams.value.endTime);
...
const endPickerOptions = computed(() => ({
  disabledDate: (time) => {
    if (!searchParams.value.startTime) return false;
    const startDate = new Date(searchParams.value.startTime);
    ...
    if (response.code === 1) {
      ...
      formData.value = Object.keys(formData.value).reduce((acc, key) => {
        if (
          ...
          // skcId, spuId, shopId 不做转换，直接赋值为字符串
...
// 获取商品销售件数的函数
const fetchSalesCountByProduct = async (skcId: string) => {
  // const loading = ElLoading.service({
  //   lock: true,
  //   text: "正在加载商品销售件数...",
  // });
  try {
    const response = await request.get("/api/sale/getSalesCountByProduct", {
      params: { skcId },
    });
    return response; // 返回商品的销售件数
  } catch (error) {
    console.error(`获取商品 ${skcId} 销售件数失败:`, error);
    return 0; // 如果失败，返回 0
  }
};
...
async function fetchProducts(
  ...
  try {
    const response = await request.get("/api/product/allBindProducts", {
      params: {
        nickName,
        shopId,
        page,
    ...
    products.value = response.pageData.records; // 记录
    // 获取每个商品的销售件数

    // for (const product of products.value) {
    //   const salesCount = await fetchSalesCountByProduct(product.skcId);
    //   product.salesCount = salesCount;
    // }
    originalProducts.value = response.pageData.records;
    totalProducts.value = response.pageData.total; // 总数
...
//       });
//       boundDesigners.value.set(product.skcId, response.data); //获取一个商品绑定的设计师
//     } catch (error) {
...
const checkBoundProducts = async () => {
  try {
    ...
    products.value.forEach((product) => {
      boundProducts.value.set(product.skcId, boundSkcIds.has(product.skcId)); //设置商品的绑定状态，有就为1
    });
  } catch (error) {
...
const unbindUserFromProduct = async (product) => {
  try {
    ...
    if (response.code === 1) {
      ...
      console.log("product.designerList11111:", product.designerList);
      if (product.designerList) {
      ...
      console.log("product.designerList22222:", product.designerList);
    }
  } catch (error) {
